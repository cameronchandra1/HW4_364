...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://postgres:29Malysana@localhost:5432/HW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
class Gif(db.Model):
    ...
    # TODO 364: Add code for the Gif model such tchat it has the following fields:
...
class PersonalGifCollection(db.Model):
    ...
    gif_collections = db.relationship('Gif',secondary=user_collection,backref=db.backref('personalgifCollections',lazy='dynamic'),lazy='dynamic')
...
def get_gifs_from_giphy(search_string):
    ...
    limit = 5 
    data = requests.get('http://api.giphy.com/v1/gifs/search?q={}&limit={}&api_key={}'.format(search_string,limit,api_key))
...
def get_or_create_collection(name, current_user, gif_list=[]):
    ...
    if collection:
        ...
        collection = PersonalGifCollection(name=name,user_id=current_user.id,gif_collections=[])
        for gif in gif_list:
            collection.gif_collections.append(gif)
...
def create_collection():
    ...
    if request.method == 'POST':
        gif_list = form.gif_picks.data
        items = [get_gif_by_id(int(id)) for id in gif_list]
        collection = get_or_create_collection(name=form.name.data,current_user=current_user,gif_list=items)
...
def single_collection(id_num):
    ...
    gifs = collection.gif_collections.all()
